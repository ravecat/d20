...
- Make the authentication dialog, Account Settings, Registration Completion, and Magic Link Login Confirmation available for deterministic visual inspection under `Sign In` and `Sign Up` workflows without Phoenix or live Inertia submissions, with their addon tooling visible beside the desktop canvas.
...
### Organize authentication stories by user workflow

Storybook imports the production AuthDialog, Account Settings, Registration Completion, and Auth Confirmation components and supplies deterministic state for their meaningful server-owned workflows. The `Sign In` group contains the initial authentication dialog, its Magic Link request result, sudo reauthentication, and the Magic Link login-confirmation page for signed-out and reauthenticating users. The `Sign Up` group contains the initial registration dialog, its email-confirmation request result, and the two distinct registration-completion states: Magic Link and Auth Provider. The provider-backed story represents Google, Discord, and Apple because their completion pages expose the same user-visible state, including the action to choose another registration method. Account Settings remains an independent account-management page with its required username and provider availability differences.

The Storybook Vite configuration replaces only the `@inertiajs/svelte` module used inside the isolated catalog with a story-owned boundary. Its `Form` renders a normal HTML form, supplies deterministic idle slot state with no validation errors, and prevents submission. The remaining exports provide inert navigation plus resettable deterministic page props for dialog stories. Leaf component stories and AuthDialog import their leaf component and auth-store modules instead of evaluating shared barrels that also expose live session transports. The boundary does not imitate server success, failure, or navigation. Production builds and frontend tests keep their existing Inertia modules and mocks, so catalog interaction cannot contact Phoenix while the stories still exercise the real production markup and styles.
