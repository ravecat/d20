...
- Scope bonus actions to bonuses opened during the submitted turn and resolve omitted current-turn bonuses as forfeited so they cannot be saved.
...
- Affected public contract: the existing `bonus_actions` array keeps its shape, while omission explicitly forfeits remaining bonuses opened during that turn.
