...
- [x] 23.6 Synchronize affected authoritative specs, archive the same change, validate strict OpenSpec/lifecycle/active-list state, and record verified evidence in #264; the later commit/transfer request supersedes the earlier uncommitted-delivery restriction.
...
- [x] 31.3 Validate the candidate and reconcile remaining delivery gates before finalization; fast-forward master, validate the integrated target, and retire the source worktree when all transfer gates pass.
...
- Integrated-target backend verification again passed 827 tests with the same known Credo findings. The complete unit/browser target run reproduced the two narrow-home screenshot failures (166 passed, one expected skip), despite the isolated retry passing. The screenshot diff isolates the hero card hover treatment: a retained pointer position activates its existing one-pixel translation after other browser files. Section 31 therefore includes moving the pointer to the section heading before the narrow static screenshot, followed by the complete unit/browser regression run; no animation behavior, product source, or baseline changes are required. The pointer-isolation correction passed the complete unit/browser regression run: 21 files, 168 tests passed and one expected Firefox skip, without changing product code or references. The corrected integrated-target check and archival are verified in the final evidence below.


Section 31 integrated delivery evidence:

- Local master includes feature commit `3761a2f` and pointer-isolation correction `864436c`. The correction was rebased onto the concurrently delivered `f0055ac` shared Compose-router change without conflicts or product changes. Both transfers were fast-forwards, preserving the footer and independent development configuration.
- The complete corrected unit/browser suite passed 168 tests in 21 files, with one expected Firefox skip. After rebasing, targeted Chromium/Firefox home tests passed five tests with the same expected skip; the identical check passed again in primary master. Targeted ESLint and formatting passed. No screenshot baselines or application code changed for the pointer correction.
- Integrated-target backend verification passed all 827 tests. The three previously disclosed Credo nesting findings and the independently recorded Player Count Label glyph instability remain baseline limitations. The full frontend run is not claimed green; all 48 home Storybook cases passed normally, and the corrected unit/browser suite passed completely. The expensive full Storybook run was not repeated for a one-line test-isolation fix or documentation finalization.
- Six authoritative capabilities are synchronized. All implementation, migration, live-page, and integrated-target behavior gates are resolved. Archive this completed change and validate strict OpenSpec/lifecycle/active-list state in the finalization commit; then fast-forward that documentation commit and retire the clean source worktree. Keep both recovery refs and unrelated worktrees. Record final commit and lifecycle evidence in issue #264.

- Native archival completed as `2026-09-08-add-home-game-discovery-carousel` after the six-capability synchronization. `openspec validate --all --strict --no-interactive` passed all 81 remaining items, `mix openspec.check` passed for nine active changes, and `openspec list --json` confirms this change is absent. Source runtime verification found no task-owned browser-test or development server still running. Source retirement follows the clean final documentation transfer; recovery snapshots remain available.
