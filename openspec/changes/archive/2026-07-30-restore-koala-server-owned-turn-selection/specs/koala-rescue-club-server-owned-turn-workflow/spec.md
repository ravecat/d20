...
The `submit_turn_selection` command SHALL consume the caller's stored complete selection and submitted ordered bonus decisions. The server SHALL revalidate and apply the primary action, volunteer spending, bonus actions, submitted status, and selection removal atomically.
