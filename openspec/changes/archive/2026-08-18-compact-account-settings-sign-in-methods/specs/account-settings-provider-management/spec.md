...
The sudo-protected Account Settings page SHALL render an external sign-in provider only when that provider's existing page prop reports `available` as `true`. It MUST omit unavailable providers regardless of durable linked state and MUST omit the complete Sign-in methods section when no provider is available. Provider visibility MUST NOT change provider configuration, identity ownership, or linking authorization.
...
- **THEN** the page renders every available provider
