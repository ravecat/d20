Current presentation: task 3.26 supersedes earlier completed disclosure, duplicated-list and bottom-strip tasks. Those checked tasks record historical increments. Missing content and publication gates remain open.
...
- [x] 3.26 Apply the Zed reference directly in master: one always-visible heading/anchor tree without details/summary or ul/li; service block left on desktop/top on mobile with `d20 © <current year>` and guest-only `Sign up · Have an account? Sign in` actions opening the existing dialog; preserve links, themes and shell geometry. Replace disclosure tests and stories, preserve generic layout overflow-removal coverage, review affected visual references and the prepared page across widths/themes/scaling, run scoped frontend and OpenSpec checks, and commit the reconciled increment with Refs #268.
...
- [ ] 4.2 Add focused footer/page tests for group names, hrefs, consistent shared footer content, FAQ/deletion anchors, anonymous/authenticated states, empty catalog, permanently visible navigation, and lack of footer-triggered requests; run the relevant files with `bun run test:unit -- <test-path>` from `assets/`.
- [ ] 4.3 Update existing public/authenticated Home stories and add prepared-page coverage and the single Default footer story; use the existing 1280x720, 1024x640, and 320x900 presets in light/dark, including short content and focused links, with `bun run test:visual -- <story-path>`.
- [ ] 4.4 Use `$devtools-validations` to inspect desktop/mobile/desktop and mobile/desktop/mobile sequences around 48rem, same-mode orientation changes, always-visible groups and retained focused links, reduced motion, keyboard traversal, and the always-visible legal row. Include 320px reflow, 200-percent zoom, no-script navigation where footer markup is available, safe-area/Workspace interaction, and no added third-party traffic.
- [ ] 4.5 Add focused browser coverage for the transition matrix, including 767/768/769 CSS-pixel boundary checks at the default root font size, one visible anchor per destination and keyboard order, and direct `/help#delete-account` navigation; run `bun run test:browser -- <test-path>` from `assets/`.
