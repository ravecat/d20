...
Storybook currently disables production CSS motion in its shared preview and cancels animations in four Workspace stories, so contributors cannot inspect the components' real animated behavior. Restore that behavior while retaining stable state screenshots through the existing Storybook, Vitest, and Playwright test pipeline.
...
- `storybook-component-catalog`: Preserve production CSS motion during interactive story browsing and use the existing test pipeline for stable screenshots of configured story states.

## Impact

- Frontend catalog: `assets/.storybook/preview.ts`, removal of `assets/.storybook/preview.css`, and `assets/stories/widgets/workspace.stories.ts`.
