...
- Add deterministic Storybook page scenarios for Account Settings, Registration Completion, and Auth Confirmation while replacing their live Inertia form-submission boundary and keeping the desktop addon panel visible to the right of the story canvas.
...
- `storybook-component-catalog`: Make the three account pages affected by the authentication flow directly inspectable through typed, backend-independent page stories with the desktop addon panel visible beside them.
...
- Frontend: changes the shared auth dialog, Account Settings, and shared Inertia prop types while reusing the provider-neutral registration-completion page introduced by Google authentication; adds Storybook-only form isolation, page stories, and manager layout configuration for the affected account surfaces.
