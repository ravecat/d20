...
Task 21 supersedes the earlier task-specific screenshot/Storybook waiver for current visual reconciliation. The approved current icon-only rendering SHALL receive reviewed references and successful normal comparisons through the unchanged native matrix. Historical waiver and unchanged-image statements below describe earlier amendments only; unrelated manual, cross-stack, rollout and archive gates remain open until independently verified.
