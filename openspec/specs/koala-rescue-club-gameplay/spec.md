# koala-rescue-club-gameplay Specification

## Purpose
TBD - created by archiving change implement-koala-rescue-club-rules. Update Purpose after archive.
## Requirements
### Requirement: Players can start Koala Rescue Club with a selected sheet
The system SHALL initialize Koala Rescue Club from a supported session-creation sheet and SHALL allow the session owner to start after at least one player has joined.

#### Scenario: Player joins before start
- **WHEN** a player joins a Koala Rescue Club session before the game starts
- **THEN** the system adds one entry for that participant to the players map
- **AND** the player receives an empty player sheet for the aggregate's selected rulesheet

#### Scenario: Owner starts an initialized game
- **WHEN** the session owner starts a Koala Rescue Club session initialized with sheet `dharug` or `yugambeh`
- **THEN** the game keeps every accepted player on the selected sheet
- **AND** each player retains the sheet's initial volunteers
- **AND** the game enters the roll phase for turn 1

#### Scenario: Selected sheet controls later rules
- **WHEN** Koala Rescue Club has started with a selected sheet
- **THEN** every later placement, accessibility, bonus, hospital, badge, solo rating, round scoring, and final scoring decision uses that sheet's structure

#### Scenario: Unsupported sheet cannot create a session
- **WHEN** a user attempts to create Koala Rescue Club with an unsupported sheet
- **THEN** the system rejects session creation
- **AND** no game process starts
...
- **THEN** the system records each submitted player's accepted adjusted die value in that player's turn log
...
- **WHEN** a player submits a complete tree or koala shape as the primary turn result
...
- **WHEN** a player submits a one-cell tree or koala primary result
