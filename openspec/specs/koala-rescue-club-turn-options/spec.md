# koala-rescue-club-turn-options Specification

## Purpose
TBD - created by archiving change unify-koala-turn-options. Update Purpose after archive.
## Requirements
### Requirement: Caller-specific turn state uses top-level projection fields
The system SHALL expose the current caller's derived options and staged selection as top-level `options` and `selection` fields and SHALL NOT expose a `turn` envelope or the replaced `turn_options` and `turn_selection` fields.

#### Scenario: Pending player receives caller-specific turn fields
- **WHEN** a pending player receives a Koala Rescue Club session projection after the shared roll
- **THEN** the projection contains top-level `options` for that player
- **AND** top-level `selection` contains only that player's current staged selection or `null`

#### Scenario: Submitted player has no actionable options
- **WHEN** a player who already submitted receives a projection
- **THEN** top-level `options` is an empty object
...
- **THEN** top-level `options` reflects the new authoritative state without a separate options update command
...
- **THEN** each player's top-level `options` is independently derived for that player
