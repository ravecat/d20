defmodule D20Web.Auth.DiscordControllerTest do
  ...
  alias D20Web.Auth.DiscordController

  setup do
    previous_oauth_config = Application.get_env(:ueberauth, Ueberauth.Strategy.Discord.OAuth)
    ...
    on_exit(fn ->
      restore_application_env(:ueberauth, Ueberauth.Strategy.Discord.OAuth, previous_oauth_config)
    end)
  end

  describe "GET /auth/discord" do
    test "fails locally when Discord credentials are unavailable", %{conn: conn} do
      ...
      {conn, log} =
        ExUnit.CaptureLog.with_log(fn ->
          get(conn, "/auth/discord?return_to=%2Fgames%2Fqwinto")
        end)
      ...
      refute redirected_to(conn) =~ "discord.com"
      refute log =~ "discord-client-secret"
    end
    ...
    test "uses the fixed minimal scope and strips caller provider overrides", %{conn: conn} do
      conn =
        get(
          conn,
          "/auth/discord?return_to=%2Fgames%2Fqwinto&scope=guilds&prompt=consent&permissions=8&guild_id=private&disable_guild_select=true&integration_type=1&bot=true&redirect_uri=https%3A%2F%2Fevil.example&locale=pl&client_id=attacker&state=attacker"
        )

      assert redirected_to(conn, 302) =~ "https://discord.com/oauth2/authorize?"
      ...
      assert query["redirect_uri"] == "http://example.com/auth/discord/callback"
      assert is_binary(query["state"])

      for name <- ~w(prompt permissions guild_id disable_guild_select integration_type bot locale) do
        refute Map.has_key?(query, name)
      end

      assert get_session(conn, :return_to) == "/games/qwinto"
      assert %{"action" => "authenticate"} = get_session(conn, :discord_auth_intent)
    end
    ...
    test "binds reauthentication requests to the current user", %{conn: conn} do
      ...
      conn =
        conn
        |> log_in_user(user)
        |> get(~p"/auth/discord?intent=reauthenticate&return_to=/users/settings")
    ...
    test "rejects an unsafe return destination", %{conn: conn} do
      conn = get(conn, "/auth/discord?return_to=https%3A%2F%2Fevil.example%2Fsteal")
  ...
  describe "GET /auth/discord/callback" do
    test "does not exchange a callback while Discord credentials are unavailable", %{conn: conn} do
      Application.put_env(:ueberauth, Ueberauth.Strategy.Discord.OAuth,
        client_id: "discord-client-id",
        client_secret: nil
      )
      ...
      {conn, log} =
        ExUnit.CaptureLog.with_log(fn ->
          ...
          |> Discord.put_authenticate_intent()
          |> get("/auth/discord/callback?state=provider-state&code=secret-authorization-code")
        end)
    ...
    test "rejects invalid state before trusting the authorization code", %{conn: conn} do
      {conn, log} =
        ExUnit.CaptureLog.with_log(fn ->
          conn
          |> init_test_session(return_to: "/games/qwinto")
          |> get("/auth/discord/callback?state=invalid&code=secret-authorization-code")
        end)
    ...
    test "logs in an exact identity owner, rotates the session, and uses safe return", %{
      ...
      user = user_fixture()
      assert {:ok, _identity} = Accounts.link_user_identity(user, :discord, "returning-subject")

      conn =
        conn
        |> direct_callback_conn(nil,
          return_to: "/games/qwinto",
          anonymous_user_id: "anon_before_discord"
        )
        |> Discord.put_authenticate_intent()
        |> assign(:ueberauth_auth, discord_auth("returning-subject", nil, false))
    ...
    test "reauthenticates only the current user with an exact linked identity", %{conn: conn} do
      user = authenticated_user(user_fixture())
      assert {:ok, _identity} = Accounts.link_user_identity(user, :discord, "reauth-subject")

      conn =
        conn
        |> direct_callback_conn(user, return_to: "/users/settings")
        |> Discord.put_reauthenticate_intent(user)
        |> assign(:ueberauth_auth, discord_auth("reauth-subject", nil, false))
        |> DiscordController.callback(%{})
    ...
    test "does not switch accounts during provider reauthentication", %{conn: conn} do
      ...
      assert {:ok, _identity} = Accounts.link_user_identity(other_user, :discord, "other-subject")

      conn =
        conn
        |> direct_callback_conn(current_user, return_to: "/users/settings")
        |> Discord.put_reauthenticate_intent(current_user)
        |> assign(:ueberauth_auth, discord_auth("other-subject", nil, false))
        |> DiscordController.callback(%{})
    ...
    test "starts completion for an unknown subject with verified unused email", %{conn: conn} do
      ...
      conn =
        ...
        |> Discord.put_authenticate_intent()
        |> assign(:ueberauth_auth, discord_auth("new-subject", email, true))
        |> DiscordController.callback(%{})

      assert redirected_to(conn) == ~p"/auth/discord/register"
    ...
    test "starts distinct registration instead of matching an existing email", %{conn: conn} do
      ...
      conn =
        ...
        |> direct_callback_conn()
        |> Discord.put_authenticate_intent()
        |> assign(:ueberauth_auth, discord_auth("unlinked-subject", user.email, true))
        |> DiscordController.callback(%{})

      assert redirected_to(conn) == ~p"/auth/discord/register"
      refute get_session(conn, :user_token)
      refute Accounts.get_user_by_identity(:discord, "unlinked-subject")
      assert {:ok, %{email: email}} = Discord.fetch_registration(conn)
    ...
    test "starts provider-only registration without verified email", %{conn: conn} do
      conn =
        ...
        |> direct_callback_conn()
        |> Discord.put_authenticate_intent()
        |> assign(:ueberauth_auth, discord_auth("new-subject", "player@example.com", false))
        |> DiscordController.callback(%{})

      assert redirected_to(conn) == ~p"/auth/discord/register"
      refute get_session(conn, :user_token)

      assert {:ok, %{provider_uid: "new-subject", email: nil}} = Discord.fetch_registration(conn)
    end
    ...
    test "fails closed without a valid callback intent", %{conn: conn} do
      conn =
        conn
        |> direct_callback_conn()
        |> assign(:ueberauth_auth, discord_auth("new-subject", unique_user_email(), true))
        |> DiscordController.callback(%{})
  ...
  describe "Discord registration completion" do
    test "renders the shared page without exposing the Discord identity", %{conn: conn} do
      conn =
        conn
        |> init_test_session(%{})
        |> Discord.put_registration(%{
          provider_uid: "private-discord-subject",
          email: "player@example.com"
        })
        |> get(~p"/auth/discord/register")
    ...
    test "creates and authenticates exactly one complete account", %{conn: conn} do
      ...
      conn =
        conn
        |> init_test_session(return_to: "/games/qwinto")
        |> Discord.put_registration(%{provider_uid: "new-subject", email: email})
        |> post(~p"/auth/discord/register", %{
          "user" => %{"username" => "discord_player", "remember_me" => "true"}
        })

      assert redirected_to(conn) == "/games/qwinto"
      assert conn.resp_cookies["_d20_web_user_remember_me"]

      assert %User{username: "discord_player", confirmed_at: confirmed_at} =
               user = Accounts.get_user_by_email(email)

      assert confirmed_at
      assert Accounts.get_user_by_identity(:discord, "new-subject").id == user.id
      assert session_user(conn).id == user.id
      ...
      replay_conn =
        conn
        |> recycle()
        |> post(~p"/auth/discord/register", %{"user" => %{"username" => "other_player"}})
    ...
    test "creates a provider-only account when completion has no email", %{conn: conn} do
      conn =
        conn
        |> init_test_session(%{})
        |> Discord.put_registration(%{provider_uid: "provider-only-subject", email: nil})
        |> post(~p"/auth/discord/register", %{"user" => %{"username" => "discord_only"}})

      assert redirected_to(conn) == ~p"/"

      assert %User{email: nil, username: "discord_only"} =
               Accounts.get_user_by_identity(:discord, "provider-only-subject")
    end

    test "discards an already-owned email during Discord completion", %{conn: conn} do
      owner = user_fixture()
      ...
      conn =
        ...
        |> post(~p"/auth/discord/register", %{"user" => %{"username" => "discord_distinct"}})

      assert redirected_to(conn) == ~p"/"

      assert %User{email: nil, username: "discord_distinct"} =
               Accounts.get_user_by_identity(:discord, "owned-email-subject")
    ...
    test "keeps valid completion state after username validation", %{conn: conn} do
      ...
      conn =
        ...
        |> Discord.put_registration(%{provider_uid: "new-subject", email: email})
        |> inertia_request()
        |> post(~p"/auth/discord/register", %{"user" => %{"username" => "invalid name"}})

      assert redirected_to(conn, 303) == ~p"/auth/discord/register"
      assert {:ok, %{provider_uid: "new-subject"}} = Discord.fetch_registration(conn)
      refute Accounts.get_user_by_email(email)
    ...
    test "clears expired completion without creating an account", %{conn: conn} do
      ...
      conn =
        ...
        |> post(~p"/auth/discord/register", %{"user" => %{"username" => "discord_player"}})
    ...
    test "clears completion when the player chooses email registration", %{conn: conn} do
      ...
      conn =
        ...
        |> init_test_session(%{})
        |> Discord.put_registration(%{provider_uid: "new-subject", email: email})
        |> post(~p"/auth/discord/register/cancel")
  ...
  describe "GET /users/settings/auth/discord" do
    test "requires authentication and sudo mode", %{conn: conn} do
      conn = get(conn, ~p"/users/settings/auth/discord")
      assert redirected_to(conn) == ~p"/"
      ...
      stale_conn =
        ...
        |> get(~p"/users/settings/auth/discord")

      assert redirected_to(stale_conn) == ~p"/"
      assert get_session(stale_conn, :auth_prompt).reauthenticate == true
    end

    test "stores a user-bound link intent and starts only through Discord request", %{conn: conn} do
      user = user_fixture()

      conn = conn |> log_in_user(user) |> get(~p"/users/settings/auth/discord")

      assert redirected_to(conn) == ~p"/auth/discord"
      assert {:ok, {:link, user_id}} = Discord.fetch_intent(conn)
      assert user_id == to_string(user.id)
    end
    ...
    test "links the subject to the same sudo-valid user", %{conn: conn} do
      ...
      conn =
        ...
        |> assign(:ueberauth_auth, discord_auth("linked-subject", nil, false))
        |> DiscordController.callback(%{})

      assert redirected_to(conn) == ~p"/users/settings"
      assert Accounts.get_user_by_identity(:discord, "linked-subject").id == user.id
      assert session_user(conn).id == user.id
    end

    test "treats the same owner as idempotent", %{conn: conn} do
      user = authenticated_user(user_fixture())
      assert {:ok, identity} = Accounts.link_user_identity(user, :discord, "linked-subject")
    ...
    test "reports one generic conflict for another owner or another subject", %{conn: conn} do
      user = authenticated_user(user_fixture())
      other_user = user_fixture()
      assert {:ok, _identity} = Accounts.link_user_identity(other_user, :discord, "owned-subject")

      conflict_conn =
        conn
        |> direct_callback_conn(user)
        |> Discord.put_link_intent(user)
        |> assign(:ueberauth_auth, discord_auth("owned-subject", nil, false))
        |> DiscordController.callback(%{})

      assert redirected_to(conflict_conn) == ~p"/users/settings"

      assert Phoenix.Flash.get(conflict_conn.assigns.flash, :error) ==
               "Discord could not be linked to this account."

      assert Accounts.get_user_by_identity(:discord, "owned-subject").id == other_user.id
    end
    ...
    test "rejects a callback bound to a different current user", %{conn: conn} do
      ...
      conn =
        ...
        |> Discord.put_link_intent(initiating_user)
        |> assign(:ueberauth_auth, discord_auth("unbound-subject", nil, false))
        |> DiscordController.callback(%{})

      assert redirected_to(conn) == ~p"/"
      refute Accounts.get_user_by_identity(:discord, "unbound-subject")
    end
  end
  ...
  defp discord_auth(subject, email, verified) do
    %Ueberauth.Auth{
      ...
      extra: %Ueberauth.Auth.Extra{
        raw_info: %{
          user: %{"id" => subject, "email" => email, "verified" => verified},
