...
```sh
mix usage_rules.search_docs "search term" -p phoenix -p phoenix_ecto -p phoenix_html -p phoenix_live_dashboard -p phoenix_live_reload -p phoenix_live_view -p phoenix_vite
```
...
- `mix phoenix_vite.install` - Installer for Phoenix Vite
- `mix phoenix_vite.install.helper`
- `mix phoenix_vite.npm` - Invokes node with the profile and args
