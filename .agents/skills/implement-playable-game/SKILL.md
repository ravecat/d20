...
- Name runtime phases after real rule-defined game states or clear domain equivalents that explain what is happening and what can happen next. Do not promote a derived condition or an internal service operation to a phase merely because the implementation needs to evaluate or execute it. Keep derived conditions in `Rules` and represent service work through events or internal transitions unless it creates a behaviorally distinct game state that projections or clients must observe.
