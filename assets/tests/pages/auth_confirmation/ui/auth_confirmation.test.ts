...
describe("magic-link confirmation page", () => {
  it("submits the existing confirmation token contract", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Confirm your account" })).not.toBeNull();
    expect(screen.getByText("player@example.com")).not.toBeNull();

    await fireEvent.click(screen.getByLabelText("Keep me signed in"));
    await fireEvent.click(screen.getByRole("button", { name: "Confirm account" }));
    ...
    expect(inertiaMock.formSubmit).toHaveBeenLastCalledWith({
      ...
      data: {
        _action: "confirmed",
        user: { remember_me: "true", token: "confirmation-token" },
      },
    });
  });
