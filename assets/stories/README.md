...
- Interactive stories retain production CSS animations, transitions, and scrolling behavior, including the application's reduced-motion rules. Do not add global motion suppression or document-wide animation cancellation to preview configuration or story interactions.
- Use args and `play` to select the state to capture; the standard Storybook, Vitest, and Playwright pipeline stabilizes automated screenshots. Stable snapshots do not verify animation progression; keep meaningful motion checks in focused browser tests.
- Storybook 10.5.7 may wait up to five seconds for animation completion on finite scroll timelines, including after theme changes. Restoring interactive motion retains this upstream waiting limitation.
